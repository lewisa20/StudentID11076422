...
s_norm_A = 0.6;     % Curve A stdDev
m_norm_B = 1.25;    % Curve B mean
s_norm_B = 0.6;     % Curve B stdDev
m_norm_C = 1.5;     % Curve C mean
s_norm_C = 0.6;     % Curve C stdDev
m_norm_D = 1.75;    % Curve D mean
s_norm_D = 0.6;     % Curve D stdDev
m_norm_E = 2.0;     % Curve E mean
s_norm_E = 0.6;     % Curve E stdDev

% Convert mean and std to beta/theta for curves A, B, C, D and E
...
% Vertical offset for text above each dot
offset = 0.05;

% Plot dot and label for Company A (blue)
plot(theta_A, 0.5, 'bo', 'MarkerFaceColor', 'b', 'MarkerSize', 8, 'HandleVisibility','off');
text(theta_A, 0.5+offset, sprintf('%.2f', theta_A), ...
    'VerticalAlignment','bottom', 'HorizontalAlignment','center', ...
    'Rotation', 90, 'FontSize',10, 'Color','k');

% Plot dot and label for Company B (red)
plot(theta_B, 0.5, 'ro', 'MarkerFaceColor', 'r', 'MarkerSize', 8, 'HandleVisibility','off');
text(theta_B, 0.5+offset, sprintf('%.2f', theta_B), ...
    'VerticalAlignment','bottom', 'HorizontalAlignment','center', ...
    'Rotation', 90, 'FontSize',10, 'Color','k');

% Plot dot and label for Company C (green)
plot(theta_C, 0.5, 'go', 'MarkerFaceColor', 'g', 'MarkerSize', 8, 'HandleVisibility','off');
text(theta_C, 0.5+offset, sprintf('%.2f', theta_C), ...
    'VerticalAlignment','bottom', 'HorizontalAlignment','center', ...
    'Rotation', 90, 'FontSize',10, 'Color','k');

% Plot dot and label for Company D (magenta)
plot(theta_D, 0.5, 'mo', 'MarkerFaceColor', 'm', 'MarkerSize', 8, 'HandleVisibility','off');
text(theta_D, 0.5+offset, sprintf('%.2f', theta_D), ...
    'VerticalAlignment','bottom', 'HorizontalAlignment','center', ...
    'Rotation', 90, 'FontSize',10, 'Color','k');

% Plot dot and label for Company E (black)
plot(theta_E, 0.5, 'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 8, 'HandleVisibility','off');
text(theta_E, 0.5+offset, sprintf('%.2f', theta_E), ...
    'VerticalAlignment','bottom', 'HorizontalAlignment','center', ...
    'Rotation', 90, 'FontSize',10, 'Color','k');


% Dots at 0.8 probability for blue (Company A) and black (Company E)
% Get the x-values corresponding to 0.8 probability
